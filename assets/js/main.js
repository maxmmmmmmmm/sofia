...
(function () {
  ...
  (function initPriceTabs() {
    ...
    tabs.forEach(function (tab) {
      tab.addEventListener("click", function () {
        var target = tab.dataset.city;
        tabs.forEach(function (t) {
          var on = t.dataset.city === target;
          t.classList.toggle("is-active", on);
          t.setAttribute("aria-selected", String(on));
        });
        $$(".sf-pricelist").forEach(function (list) {
          list.hidden = list.dataset.city !== target;
        });
      });
    });
  })();
  ...
  /* ------------------------------------------------------------------
     Галерея по рядам: сначала пять, дальше по кнопке
     ------------------------------------------------------------------ */

  (function initMore() {
    var mosaic = $(".sf-mosaic");
    var btn = $("[data-more]");
    if (!mosaic || !btn) return;

    var ROWS_STEP = 5;
    var items = $$(".sf-mosaic__item", mosaic);
    var shown = 0;

    // Ряды не заданы разметкой — их складывает флексбокс, и на разной ширине
    // они разные. Поэтому границы рядов вычисляем по фактическому положению
    // карточек: одинаковый offsetTop = один ряд.
    function itemsForRows(rowCount) {
      var tops = [], n = 0;
      for (var i = 0; i < items.length; i++) {
        var top = items[i].offsetTop;
        if (tops.indexOf(top) === -1) {
          if (tops.length === rowCount) break;
          tops.push(top);
        }
        n = i + 1;
      }
    ...
    function recount() {
      // считаем на полностью открытой галерее, иначе рядов не видно
      items.forEach(function (el) { el.hidden = false; });
      var target = itemsForRows(ROWS_STEP);
      apply(Math.max(target, shown || target));
    }

    btn.addEventListener("click", function () {
      items.forEach(function (el) { el.hidden = false; });
      var next = itemsForRows(
        // сколько рядов уже показано + ещё пять
        (function () {
          var tops = [];
          for (var i = 0; i < shown; i++) {
            var tp = items[i].offsetTop;
            if (tops.indexOf(tp) === -1) tops.push(tp);
          }
          return tops.length + ROWS_STEP;
        })()
      );
      apply(next);
    });
    ...
    window.addEventListener("resize", function () {
      if (shown >= items.length) return;   // всё открыто — пересчитывать нечего
      recount();
    });
  ...
  /* ------------------------------------------------------------------
     Главная на телефоне: название раздела проявляется, когда карточка
     выходит на середину экрана
     ------------------------------------------------------------------ */
  ...
  (function initCardTouch() {
    ...
    cards.forEach(function (card) {
      ...
      // Палец поехал — значит это прокрутка, а не выбор раздела.
      card.addEventListener("touchmove", function (e) {
        if (startY !== null && Math.abs(e.touches[0].clientY - startY) > 10) clearAll();
      }, { passive: true });

      // На отпускании подсветку НЕ снимаем: она держится до касания другой
      // карточки. Иначе на переходе получалось полусостояние — вуали уже
      // нет, а название ещё не догасло.
      card.addEventListener("touchcancel", clearAll, { passive: true });
    });
  })();
