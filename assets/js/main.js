...
(function () {
  ...
  /* ------------------------------------------------------------------
     Loading veil — show it once per visit, not on every page click
     ------------------------------------------------------------------ */

  (function initLoader() {
    var veil = $(".sf-loader");
    if (!veil) return;      // повторный заход: её не отрисовали вовсе

    // Помечаем визит, чтобы следующая страница обошлась без заставки.
    try { sessionStorage.setItem("sf-seen", "1"); } catch (e) { /* private mode */ }

    // Снимаем слой, когда анимация доиграла.
